...
// perfetto example code
void collect_power_rails(std::atomic<bool>& sigterm) {
    // Warning!!!
    // Perfetto is only supported on Google Android platform (especially for Pixel series).
    // Please make sure you run this code on a compatible device.

#if defined(PERFETTO_IMPLEMENTATION)
    // use system trace backend
    perfetto::TracingInitArgs args;
    args.backends = perfetto::kSystemBackend;
    perfetto::Tracing::Initialize(args);

    // construct TraceConfig
    perfetto::TraceConfig cfg;
    cfg.add_buffers()->set_size_kb(1024); // 1MB buffer

    // --- android.power ---
    {
        auto* dsc = cfg.add_data_sources()->mutable_config();
        ...
        // (CHARGE=1, CAPACITY_PERCENT=2, CURRENT=3, CURRENT_AVG=4, VOLTAGE=5, etc.) :contentReference[oaicite:9]{index=9}
    ...
    // trace output :contentReference[oaicite:12]{index=12}
...
int main(int argc, char **argv) {
...
#if !defined(PERFETTO_IMPLEMENTATION)
    std::cerr << 
        "Perfetto is not enabled in this build. Please enable PERFETTO macro in CMakeLists.txt to use perfetto features."
              << std::endl;
    return 1;
#endif
    ...
    std::thread perfetto_thread = std::thread(collect_power_rails, std::ref(sigterm));
